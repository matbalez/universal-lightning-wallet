...
5. Parse the raw BOLT12 offer from the `offer` tag and verify it matches the recipient's NIP-100 public BOLT12 receive offer.
6. Decode and validate the `lnp` payer proof according to the BOLT12 payer-proof specification.
7. Verify the payer proof cryptography:
   - `SHA256(proof_preimage)` equals `invoice_payment_hash`;
   - the reconstructed invoice Merkle root validates `signature` with `invoice_node_id`;
   - `proof_signature` validates with `invreq_payer_id`.
8. Verify the payer proof binds to this zap:
   - the x-only form of `invreq_payer_id` equals the zap event and zap intent event `pubkey`;
   - `invreq_payer_note` equals `nostr:nip102:<zap-intent-event-id>`;
...
9. Verify every offer TLV required by this NIP is disclosed and matches the zap event's raw offer.
